...
#Region "导出函数给框架并取到两个参数值"
    ...
    Public Shared Function apprun(<MarshalAs(UnmanagedType.LPStr)> ByVal apidata As String, <MarshalAs(UnmanagedType.LPStr)> ByVal pluginkey As String) As IntPtr
        ...
        App_Info.appname = "崩溃重启"
        App_Info.author = "网中行"
        App_Info.describe = "框架崩溃重启插件"
        App_Info.appv = "1.0.1"
            ...
            Case EventTypeEnum.This_SignInSuccess
                ...
                Dim NewThread As New Thread(Sub()
                                                For Each p As Process In Process.GetProcesses()
                                                    ...
                                                    End If
                                                Next
                                                Dim ExePath = Path.GetDirectoryName(System.Diagnostics.Process.GetCurrentProcess().MainModule.FileName) + "\小栗子框架.exe"
                                                If File.Exists(myBatFilePath) Then
                                                    File.Delete(myBatFilePath)
                                                End If
                                                Using w As New StreamWriter(New FileStream(myBatFilePath, FileMode.OpenOrCreate), Encoding.UTF8)
                                                    w.WriteLine("@echo off")
                                                    w.WriteLine("REM")
                                                    w.WriteLine("chcp 65001")
                                                    w.WriteLine("SETLOCAL EnableExtensions")
                                                    w.WriteLine(":begain")
                                                    w.WriteLine("set EXE=小栗子框架.exe")
                                                    w.WriteLine("FOR /F %%x IN ('tasklist /NH /FI ""IMAGENAME eq %EXE%""') DO IF %%x == %EXE% goto FOUND")
                                                    w.WriteLine("goto FIN")
                                                    w.WriteLine(":FOUND")
                                                    w.WriteLine("goto begain")
                                                    w.WriteLine(":FIN")
                                                    w.WriteLine(":start")
                                                    w.WriteLine("start /w " + ExePath)
